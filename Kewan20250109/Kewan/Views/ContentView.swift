...
struct ContentView: View {
    ...
    private func initializeBubbles() {
        ...
        let mainBubbles = [
            ...
            (title: "中小学英语", color: Color(red: 0.6, green: 0.8, blue: 0.2), size: 130.0),
            (title: "进阶高级英语", color: Color(red: 0.6, green: 0.4, blue: 0.9), size: 155.0),
            (title: "每日短语", color: Color(red: 0.1, green: 0.4, blue: 0.8), size: 100.0),
            (title: "神奇数学", color: Color(red: 0.2, green: 0.4, blue: 0.6), size: 80.0),  // 修正大小和颜色
            (title: "僵尸爱英语", color: Color(red: 0.6, green: 0.2, blue: 0.8), size: 80.0)  // 修正大小和颜色
        ].map { bubble in
            FloatingBubble(
                title: bubble.title,
                color: bubble.color,
        ...
        if bubble.title.isEmpty {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                if bubble.title == "基础单词" {
                    loadLevel1Data()
                } else if bubble.title == "中小学英语" {
                    loadLevel2Data()
                } else if bubble.title == "进阶高级英语" {
                    loadLevel3Data()
                } else if bubble.title == "每日短语" {
                    showPhraseGameView = true
                } else if bubble.title == "神奇数学" || bubble.title == "僵尸爱英语" {
                    // 更新气泡文本
                    if !clickedBubbles.contains(bubble.title) {
                        let originalTitle = bubble.title
                        if let index = bubbles.firstIndex(where: { $0.id == bubble.id }) {
                            var updatedBubble = bubble
                            updatedBubble.title = "疯狂敲码中"
                            bubbles[index] = updatedBubble
                            
                            // 2秒后恢复原来的显示
                            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                                if let index = bubbles.firstIndex(where: { $0.id == bubble.id }) {
                                    var restoredBubble = bubbles[index]
                                    restoredBubble.title = originalTitle
                                    bubbles[index] = restoredBubble
                                }
                            }
                        }
                    }
