...
class GameViewModel: ObservableObject {
    ...
    private func saveGameScore() {
        ...
        // 保存当前轮次的分数，保留一位小数
        let roundedScore = round(currentRoundScore * 10) / 10
        coreDataManager.saveGameScore(
            score: Int16(roundedScore * 10),  // 将分数乘以10存储为整数
            level: Int16(isHardMode ? 1 : 0),
            totalTime: totalTime
        )
        print("保存游戏分数: \(String(format: "%.1f", roundedScore)) 分, 模式: \(isHardMode ? "困难" : "简单"), 阶段: \(isListeningMode ? "听力" : "识字")")
    ...
    private func saveLearnedWord(_ word: MyModel) {
        // 检查单词是否已存在
        let existingWords = coreDataManager.fetchLearnedWords()
        if let existingWord = existingWords.first(where: { $0.english == word.english }) {
            // 如果单词已存在，增加复习次数
            coreDataManager.updateWordReviewCount(existingWord)
        } else {
            // 如果是新单词，保存到 CoreData
            coreDataManager.saveLearnedWord(english: word.english, chinese: word.chinese)
        }
    }
